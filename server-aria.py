...
def webhook():
    ...
    try:
        ...
        if user_message.startswith('TESTDAY'):
            ...
            if len(parts) == 2 and parts[1].isdigit():
                ...
                try:
                    ...
                    print(f'[ARIA] TESTDAY update response: {response.text}')
        ...
        if user_message == 'TEST_D7':
            ...
            if user_id in d7_conversations:
                print(f'[ARIA] Clearing old d7_conversations for {user_id}')
                del d7_conversations[user_id]
        ...
        if user_id in d7_conversations:
            ...
            if turn <= 3:
                ...
                print(f'[ARIA] D7 turn {turn} completed, next turn: {d7_conversations[user_id]}')
        ...
        print(f'[ARIA] Webhook error: {str(e)}')
        import traceback
        traceback.print_exc()
...
def call_dify(group, message, user_id):
    ...
    try:
        ...
        if user_id in user_conversations:
            ...
            print(f'[ARIA] Using conversation: {user_conversations[user_id]}')
        else:
