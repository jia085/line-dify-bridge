...
# 8 組 Dify App 的 API Keys（A-D 男性，E-H 女性）
DIFY_KEYS = {
    'A': os.environ.get('DIFY_KEY_A'),  # 男性協作型
    'B': os.environ.get('DIFY_KEY_B'),  # 男性攻擊型
    'C': os.environ.get('DIFY_KEY_C'),  # 男性遷就型
    'D': os.environ.get('DIFY_KEY_D'),  # 男性迴避型
...
# 後續腳本（只需 4 組，因為 A/E、B/F、C/G、D/H 人格相同）
D7_SCRIPTS = {
    'A': {  # 協作型（A 和 E 共用）
    ...
    try:
        ...
        if user_message.startswith('TESTDAY'):
            ...
            if len(parts) == 2 and parts[1].isdigit():
                ...
                # Day 1 = 驗證當天，所以要減去 (target_day - 1) 天
